...
export async function PATCH(request) {
  const sql = getDb();
  const { id, date, vendor, amount, entity, category } = await request.json();
  const rows = await sql`UPDATE expenses SET date=${date}, vendor=${vendor}, amount=${amount}, entity=${entity}, category=${category} WHERE id=${id} RETURNING *`;
  return NextResponse.json(rows[0]);
}
